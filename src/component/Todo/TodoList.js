  ...
  const addTask = () => {
    ...
    if (task.length > 0) {
      if (edit) {
        ...
        setTaskList([
          ...taskList.map((item) => (item.id === editRow ? payload : item)),
        ]);
        setEdit(true);
      }
      setTask("");
    ...
    >
      ...
      >
        ...
        >
          ...
          <button style={{ ...buttonStyle, color: "green" }}>Completed</button>
          <button style={{ ...buttonStyle, color: "red" }}>Pending</button>
        </div>
        <div
          style={{
            display: "flex",
            ...
            taskList.map((item, index) => {
              return (
                <div key={item.id} style={cardStyle}>
                  ...
                  <div>
                    <span>{index + 1} .</span> <span>{item.task}</span>
                  </div>
                  <div
                    style={{
                      width: "100%",
